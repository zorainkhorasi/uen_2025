...
class Rs_datacollection extends Controller
{
    ...
    public function index()
    {
        ...
        if ($data['permission'][0]->CanView == 1) {
            ...
            $randomizedCollection = collect($getClustersProvince);
            $distIdExists = $randomizedCollection->contains('dist_id', 432);
            if($distIdExists){
                $newObject = new stdClass();
                $newObject->dist_id = 432;
                $newObject->district = 'Jaffarabad-Naseerabad';
                $newObject->randomized_c = $randomizedCollection->where('dist_id', 432)->sum('randomized_c');
                $randomization[8] = $newObject;
            }


            //echo '<pre>';print_r($randomization);die;
    ...
    public function dataCollection_detail(Request $request)
    {
        ...
        if ($data['permission'][0]->CanView == 1) {
            ...
            foreach ($get_dc_table as $row) {
                ...
                $get_dc[$ke]['not_completed_forms'] = 0;
                $get_dc[$ke]['not_visited_forms'] = 0;
            }
            foreach ($get_dc_forms as $row2) {
                ...
                if (isset($get_dc[$key]) && $get_dc[$key] != '') {
                    if ($row2->istatus == 1) {
                        ...
                        $get_dc[$key]['not_completed_forms']++;
                    }
                }

            }
            $Not_visited= datacollection_model::get_Not_visited_count_by_cluster(array_keys($get_dc));
            foreach ($Not_visited as $row2) {
                $key = $row2->hh02;
                if (isset($get_dc[$key]) && $get_dc[$key] != '') {
                    $get_dc[$key]['not_visited_forms']= $row2->count;
                }
    ...
    public function dataCollection_statusdetail(Request $request)
    {
        ...
        if ($data['permission'][0]->CanView == 1) {
            ...
            $get_visited_count = datacollection_model::get_visited_count($searchdata);

            foreach ($get_dc_forms_status   as $key =>  $v){
                foreach ($get_visited_count as $s){
                    if($s->hhid==$v->hhid){
                        $get_dc_forms_status[$key]->visit_count=$s->visit_count;
                    }
                }
            }
    ...
    public function notVisit_detail(Request $request)
    {
        $data = array();
        $data['permission'] = Settings_Model::getUserRights(Auth::user()->idGroup, '', 'rs_data_collection');
        /*==========Log=============*/
        ...
        if ($data['permission'][0]->CanView == 1) {
            ...
            /*==============Data Table==============*/
            $searchdata['type'] = request()->type;
            $get_dc_forms_status = datacollection_model::notVisit_detail($searchdata);

            $data['get_dc_table'] = $get_dc_forms_status;
            return view('rapid_survey.datacollection_not_visited_detail', ['data' => $data]);
        } else {
            $trackarray["mainResult"] = "Error";
            $trackarray["result"] = "View Error - Access denied";
            Custom_Model::trackLogs($trackarray, "all_logs");
