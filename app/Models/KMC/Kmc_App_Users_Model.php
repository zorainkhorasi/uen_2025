...
class Kmc_App_Users_Model extends Model
{
    use HasFactory;
    ...
    {
        $sql = DB::connection('sqlsrv_kmc')->table('AppUser')->select('*');
        $sql->where('enabled', '=', '1')->where('proj','=', 'KMC')->orderByDesc('id');
        $data = $sql->get();
        return $data;
    }
    ...
    {
        $data = DB::connection('sqlsrv_kmc')->table('AppUser')
            ->where('enabled', '=', '1')
            ->where('proj','=', 'KMC')
    ...
    {
        $sql = DB::connection('sqlsrv_kmc')->table('District')->select('district_code as dist_id', 'district_name as district');
