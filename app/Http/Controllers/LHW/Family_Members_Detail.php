...
use App\Models\LHW\Family_Members_Detail_Model;
use App\Models\LHW\Lhw_App_Users_Model;
use App\Models\Settings_Model;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class Family_Members_Detail extends Controller
{
    ...
    public function index()
    {
        ...
        /*==========Log=============*/
        $trackarray = array(
            "activityName" => "Family_Members_Detail",
            "action" => "View Family Members Detail Summary -> Function: Family_Members_Detail/index()",
            "PostData" => "",
            "affectedKey" => "",
            "idUser" => Auth::user()->id,
        ...
        if ($data['permission'][0]->CanView == 1) {
            ...
            $data["getMainSummary"] = Family_Members_Detail_Model::getData($searchFilter);

            $trackarray["mainResult"] = "Success";
            $trackarray["result"] = "View Success";
            Custom_Model::trackLogs($trackarray, "all_logs");
            return view('lhw.family_member_details', ['data' => $data]);
        } else {
