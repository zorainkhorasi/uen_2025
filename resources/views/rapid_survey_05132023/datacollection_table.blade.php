                ...
                <div class="card total-users">
                    <div class="card-header card-no-border">
                        ...
                        <div class="card-header-right">
                            <ul class="list-unstyled card-option">
                                ...
                                <li><i class="icofont icofont-maximize full-card"></i></li>
                                <li><i class="icofont icofont-minus minimize-card"></i></li>
                                <li onclick="randomized_chart()"><i class="icofont icofont-refresh reload-card"></i>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <div class="card-body pt-0">
                        <div class="apex-chart-container goal-status text-center row">
                            <div class="rate-card col-xl-12">
                                <div class="goal-chart">
                                    <div id="randomized_cluster"></div>
                                </div>
                                <div class="goal-end-point">
                                    <ul>
                                        <li class="mt-0 pt-0">
                                            <h6 class="font-primary">Randomized</h6>
                                            <h6 class="f-w-400">
                                                @if (isset($data['randomization_total']) && $data['randomization_total'] != '')
                                                    @php
                                                        $perc_cal = ($data['randomization_total'] / $data['totalcluster']) * 100;
                                                        $perc=(isset($perc_cal) && $perc_cal!=''?$perc_cal:'0');
                                                    @endphp
                                                    {{$data['randomization_total']}}
                                                @endif
                                                <input type="hidden" id="randomized_hidden"
                            ...
                            @php
                                $s=0;
                            @endphp
                            @if (isset($data['randomization']) && $data['randomization'] != '')
                                @foreach ($data['randomization'] as $keys=>$randomization)
                                    @php
                                        $s++;
                                        $t=(isset($data['total'][$keys]['count']) && $data['total'][$keys]['count']!=''?$data['total'][$keys]['count']:'100');
                                        $d=(isset($randomization['count']) && $randomization['count']!=''?$randomization['count']:'0');
                                        $perc_cal = ($d / $t) * 100;
                                    ...
                                    <div class="project-status mt-4">
                                        <div class="media mb-0">
                                            <div class="media-body"><h6 class=" text-start"><span
                                                        class="font-primary">{{$randomization['district']}}</span></h6>
                                            </div>
                                            <p class="text-end">{{$randomization['count']}}</p>
                                        </div>
                                        <div class="progress" style="height: 10px">
                                            <div class="progress-bar-animated mysmall_text bg-{{$colors[$s]}}"
                                                 role="progressbar"
                                                 style="width: {{number_format($perc,0)}}%"
                                                 aria-valuenow="{{$randomization['count']}}" aria-valuemin="0"
                        ...
                        <div class="apex-chart-container goal-status text-center row">
                            <div class="rate-card col-xl-12">
                                ...
                                <div class="goal-end-point">
                                    <ul>
                                        <li class="mt-0 pt-0">
                                            <h6 class="font-primary">InProgress</h6>
                                            <h6 class="f-w-400">
                        ...
                        <div class="table-responsive2">
                            <table class="display datatables table-striped table-hover" id="datatable_custom">
                                ...
                                <tr>
                                    ...
                                    <th width="9%">HH Visited/Collected</th>
                                    <th width="9%">Completed</th>
                                    <th width="9%">Refused</th>
                                    <th width="9%">Households with No Elig Respondent</th>
                                ...
                                @if (isset($data['get_dc_table']) && $data['get_dc_table'] != '')
                                    @foreach ($data['get_dc_table'] as $k=>$r)
                                        ...
                                        <tr>
                                            ...
                                            <td width="9%"><a href="{{route('rs_randomized_detail').'/'.$cluster_no}}" target="_blank" >{{isset($r['hh_randomized']) && $r['hh_randomized']!=''?$r['hh_randomized']:'0'}}</a></td>
                                            <td width="9%"><a href="{{route('rs_data_collection_statusdetail').'/t/'.$cluster_no}}" target="_blank" >{{isset($r['hh_collected']) && $r['hh_collected']!=''?$r['hh_collected']:'0'}}</a></td>
                                            <td width="9%"><a href="{{route('rs_data_collection_statusdetail').'/c/'.$cluster_no}}" target="_blank" >{{isset($r['completed_forms']) && $r['completed_forms']!=''?$r['completed_forms']:'0'}}</a></td>
                                            <td width="9%"><a href="{{route('rs_data_collection_statusdetail').'/r/'.$cluster_no}}" target="_blank" >{{isset($r['refused_forms']) && $r['refused_forms']!=''?$r['refused_forms']:'0'}}</a></td>
                                            <td width="9%"><a href="{{route('rs_data_collection_statusdetail').'/n/'.$cluster_no}}" target="_blank" >{{isset($r['not_elig']) && $r['not_elig']!=''?$r['not_elig']:'0'}}</a></td>
                                            <td width="9%"><a href="{{route('rs_data_collection_statusdetail').'/o/'.$cluster_no}}" target="_blank" >{{isset($r['remaining_forms']) && $r['remaining_forms']!=''?$r['remaining_forms']:'0'}}</a></td>
                                            <td width="9%">
                                                @php
                                                    $status = '';
                                                    if ($r['hh_randomized'] > 0) {
                                                          if ($r['hh_collected']>=$r['hh_randomized'] ) {
                                                            $status = '<a href="javascript:void(0)" class="btn btn-xs btn-success text-center">Completed</a> ';
                                                        } else {
                                                             $status = '<a href="javascript:void(0)" class="btn btn-xs btn-primary text-center">In Progress</a> ';
                                                        }
                                                    } else {
                                                       $status = '<a href="javascript:void(0)" class="btn btn-xs btn-danger text-center">Not Randomized</a> ';
                                                    }
                                                    echo $status;
                                                @endphp
                                            </td>
...
@section('script')
    ...
    <script>
        ...
        function randomized_chart() {
            var randomized_hidden = $('#randomized_hidden').val();
            if (randomized_hidden == '' || randomized_hidden == undefined) {
                randomized_hidden = 0;
            }
            var options = {
                series: [randomized_hidden],
                chart: {
                    height: 350,
                    type: 'radialBar',
                    offsetY: -10,
            ...
            var chart = new ApexCharts(document.querySelector("#randomized_cluster"),
                options
            );
            chart.render();
        }
