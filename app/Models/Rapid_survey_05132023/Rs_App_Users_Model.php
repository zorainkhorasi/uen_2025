...
class Rs_App_Users_Model extends Model
{
    ...
    {
        $sql =DB::connection('sqlsrv_rapid_survey')->table('users')->select('*');
    ...
    {
        $data =DB::connection('sqlsrv_rapid_survey')->table('users')
            ->where('enabled', '=', '1')
    ...
    {
        $sql =DB::connection('sqlsrv_rapid_survey')->table('clusters')->select('dist_id','district');
