...
use App\Models\LHW\Lhw_Summary_Model;
use App\Models\Settings_Model;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\DB;

class Lhw_Summary extends Controller
{
    ...
    {
        $data = array();
        $data['permission'] = Settings_Model::getUserRights(Auth::user()->idGroup, '', 'lhw_summary');
        /*==========Log=============*/
        $trackarray = array(
            "activityName" => "Lhw_Summary",
            "action" => "View LHW Summary -> Function: Lhw_Summary/index()",
            "PostData" => "",
            "affectedKey" => "",
            "idUser" => Auth::user()->id,
        ...
        if ($data['permission'][0]->CanView == 1) {
            ...
            $data["getMainSummary"] = Lhw_Summary_Model::getLhwSummary($searchFilter);


            $trackarray["mainResult"] = "Success";
            $trackarray["result"] = "View Success";
            Custom_Model::trackLogs($trackarray, "all_logs");
            return view('lhw.lhw_summary', ['data' => $data]);
        } else {
