...
use App\Models\Rapid_survey\Rs_App_Users_Model;
...
class Rs_App_Users extends Controller
{
    ...
    {
        $data = array();
        $data["districts"] = Rs_App_Users_Model::getDistricts();
        $data["data"] = Rs_App_Users_Model::getAllData();
        $data['permission'] = Settings_Model::getUserRights(Auth::user()->idGroup, '', 'Rs_App_Users');
        /*==========Log=============*/
        $trackarray = array(
            "activityName" => "Rs_App_Users",
            "action" => "View Rs_App_Users -> Function: Rs_App_Users/index()",
            "PostData" => "",
            "affectedKey" => "",
            "idUser" => Auth::user()->id,
        ...
        if ($data['permission'][0]->CanView == 1) {
            ...
            return view('rapid_survey.rs_app_users', ['data' => $data]);
        } else {
    ...
    public function addAppUsers(Request $request)
    {
        ...
        $checkName = Rs_App_Users_Model::checkName($array['username']);
        if (count($checkName) == 0) {
            if (DB::connection('sqlsrv_rapid_survey')->table('users')->insert($array)) {
        ...
        $trackarray = array(
            "activityName" => "Rs_App_Users",
            "action" => "Add Rs_App_Users -> Function: Rs_App_Users/addAppUsers()",
            "mainResult" => $result[0],
            "result" => $result[1],
            "PostData" => $array,
            "affectedKey" => 'id',
    ...
    {
        $id = $request->input('id');
        $getUserData = Rs_App_Users_Model::getUserDetails($id);
    ...
    public function editAppUsers(Request $request)
    {
        ...
        if (isset($id) && $id != '') {
            ...
            $updateQuery = DB::connection('sqlsrv_rapid_survey')->table('users')
                ->where('id', $id)
                ->update($array);
            if ($updateQuery) {
    ...
    public function resetPwd(Request $request)
    {
        ...
        $trackarray = array(
            "activityName" => "Rs_App_Users",
            "action" => "Edit Rs_App_Users -> Function: Rs_App_Users/resetPwd()",
            "mainResult" => $result[0],
            "result" => $result[1],
            "PostData" => $array,
            "affectedKey" => 'id=' . $id,
    ...
    public function deleteAppUsers(Request $request)
    {
        ...
        $trackarray = array(
            "activityName" => "Rs_App_Users",
            "action" => "Delete Rs_App_Users -> Function: Rs_App_Users/deleteAppUsers()",
            "mainResult" => $result[0],
            "result" => $result[1],
            "PostData" => $array,
            "affectedKey" => 'id=' . $id,
