...
namespace App\Http\Controllers\KMC;

use App\Http\Controllers\Controller;
use App\Models\Custom_Model;
use App\Models\KMC\Kmc_App_Users_Model;
use App\Models\Settings_Model;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\DB;

class Kmc_App_Users extends Controller
{
    ...
    {
        $data = array();
        $data["districts"] = Kmc_App_Users_Model::getDistricts();
        $data["data"] = Kmc_App_Users_Model::getAllData();
        $data['permission'] = Settings_Model::getUserRights(Auth::user()->idGroup, '', 'kmc_app_users');
        /*==========Log=============*/
        $trackarray = array(
            "activityName" => "Kmc_App_Users",
            "action" => "View Kmc_App_Users -> Function: Kmc_App_Users/index()",
            "PostData" => "",
            "affectedKey" => "",
            "idUser" => Auth::user()->id,
        ...
        if ($data['permission'][0]->CanView == 1) {
            ...
            return view('kmc.kmc_app_users', ['data' => $data]);
        } else {
    ...
    public function addAppUsers(Request $request)
    {
        ...
        $checkName = Kmc_App_Users_Model::checkName($array['username']);
        if (count($checkName) == 0) {
            if (DB::connection('sqlsrv_kmc')->table('AppUser')->insert($array)) {
        ...
        $trackarray = array(
            "activityName" => "Kmc_App_Users",
            "action" => "Add Kmc_App_Users -> Function: Kmc_App_Users/addAppUsers()",
            "mainResult" => $result[0],
            "result" => $result[1],
            "PostData" => $array,
            "affectedKey" => 'id',
    ...
    {
        $id = $request->input('id');
        $getUserData = Kmc_App_Users_Model::getUserDetails($id);
    ...
    public function editAppUsers(Request $request)
    {
        ...
        if (isset($id) && $id != '') {
            ...
            $updateQuery = DB::connection('sqlsrv_kmc')->table('AppUser')
                ->where('id', $id)
                ->update($array);
            if ($updateQuery) {
        ...
        $trackarray = array(
            "activityName" => "Kmc_App_Users",
            "action" => "Edit Kmc_App_Users -> Function: Kmc_App_Users/editAppUsers()",
            "mainResult" => $result[0],
            "result" => $result[1],
            "PostData" => $array,
            "affectedKey" => 'id=' . $id,
    ...
    public function resetPwd(Request $request)
    {
        ...
        $trackarray = array(
            "activityName" => "Kmc_App_Users",
            "action" => "Edit Kmc_App_Users -> Function: Kmc_App_Users/resetPwd()",
            "mainResult" => $result[0],
            "result" => $result[1],
            "PostData" => $array,
            "affectedKey" => 'id=' . $id,
    ...
    public function deleteAppUsers(Request $request)
    {
        ...
        $trackarray = array(
            "activityName" => "Kmc_App_Users",
            "action" => "Delete Kmc_App_Users -> Function: Kmc_App_Users/deleteAppUsers()",
            "mainResult" => $result[0],
            "result" => $result[1],
            "PostData" => $array,
            "affectedKey" => 'id=' . $id,
