...
class Household_Summary extends Controller
{
    ...
    public function index()
    {
        ...
        /*==========Log=============*/
        $trackarray = array(
            "activityName" => "Household_Summary",
            "action" => "View LHW Household Summary -> Function: Household_Summary/index()",
            "PostData" => "",
            "affectedKey" => "",
            "idUser" => Auth::user()->id,
        ...
        if ($data['permission'][0]->CanView == 1) {
            ...
            $data["getMainSummary"] = Household_Summary_Model::getHHSummary($searchFilter);

            $trackarray["mainResult"] = "Success";
            $trackarray["result"] = "View Success";
            Custom_Model::trackLogs($trackarray, "all_logs");
            return view('lhw.household_summary', ['data' => $data]);
        } else {
